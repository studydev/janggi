...
export { generateGungMoves, generateSaMoves } from './palace'
...
export function generatePseudoMoves(board: Board, position: Position): Position[] {
  ...
  switch (piece.type) {
    case 'CHA': return generateChaMoves(board, position)
    case 'PO': return generatePoMoves(board, position)
    case 'MA': return generateMaMoves(board, position)
    case 'SANG': return generateSangMoves(board, position)
    case 'GUNG': return generateGungMoves(board, position)
    case 'SA': return generateSaMoves(board, position)
    case 'JOL': return generateJolMoves(board, position)
  }
}
