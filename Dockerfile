...
# ---------------------------------------------------------------------------
# 1) build: 7개 장기 구현을 각각 서브패스 base 로 빌드해 하나의 정적 사이트로 합친다.
# ---------------------------------------------------------------------------
...
RUN set -eux; \
    for d in claude_opus5 claude_sonnet5 luna opus5 sol sonnet5 terra; do \
...
LABEL org.opencontainers.image.title="Janggi implementations portal" \
      org.opencontainers.image.description="같은 명세로 만든 7개 웹 장기 구현을 경로별로 서빙하는 정적 포털" \
